...
# 分离名字电话
def div_name_phone(s: str):
    # 提取姓名
    name_ = s[0:s.index(',')]
    s = s[s.index(',') + 1:]
    # 搜索11位手机号并转换str 避免只搜数字有门牌号在手机号前的bug
    number = "".join(re.findall("1[0-9]{10}", s))
    i = s.find(number)
    j = s.find(number) + len(number)
    phone_ = s[i:j]
    # 去除地址中的号码
    s = s[:i] + s[j:]
    return name_, phone_, s


# 查找省
def search_province(s: str):
    for province in js.keys():
        if province.find(s) != -1:
            return province
    province = ""
    return province
...
# 分离地址
def div_address(add: str, op: int):
    municipality = ['北京', '上海', '重庆', '天津']
    # 搜索省
    add1 = search_province(add[:2])
    # 删除省份信息
    if add1 != "" and add1 not in municipality:
        i = 0
        while i < len(add1) and add1[i] == add[i]:
            i = i + 1
        add = add[i:]

    # 搜索市
    add2 = search_city(add1, add[:2])
    # 删除市信息
    if add2 != "":
        i = 0
        while i < len(add2) and add2[i] == add[i]:
            i = i + 1
        add = add[i:]
    # 有区/县/县级市字
    # 无市必有县
    if add2 == "":
        if add.find("县") != -1:
            add3 = add[:add.find("县") + 1]
            add = add[add.find("县") + 1:]
        elif add.find("区") != -1 and add.find("区") < 6:
            add3 = add[:add.find("区") + 1]
            add = add[add.find("区") + 1:]
        elif add.find("市") != -1 and add.find("市") < 6:
            add3 = add[:add.find("市") + 1]
            add = add[add.find("市") + 1:]
    else:
        # 搜索前两个字
        add3 = search_area(add1, add2, add[:2])
        # 删除区信息
        if add3 != "":
            i = 0
            while i < len(add3) and add3[i] == add[i]:
                i = i + 1
            add = add[i:]
    # 查找街/镇/乡
    if add.find("镇") != -1:
        add4 = add[:add.find("镇") + 1]
        add = add[add.find("镇") + 1:]
    elif add.find("乡") != -1:
        add4 = add[:add.find("乡") + 1]
        add = add[add.find("乡") + 1:]
    elif add.find("街道") != -1:
        add4 = add[:add.find("街道") + 2]
        add = add[add.find("街道") + 2:]
    elif add.find("园") != -1:
        add4 = add[:add.find("园") + 1]
        add = add[add.find("园") + 1:]
    else:
        add4 = ""
    ...
    if op == '1':
        ...
        if add.find("路") != -1:
            add5 = add[:add.find("路") + 1]
            add = add[add.find("路") + 1:]
        elif add.find("街") != -1:
            add5 = add[:add.find("街") + 1]
            add = add[add.find("街") + 1:]
        elif add.find("巷") != -1:
            add5 = add[:add.find("巷") + 1]
            add = add[add.find("巷") + 1:]
        elif add.find("村") != -1:
            add5 = add[:add.find("村") + 1]
            add = add[add.find("村") + 1:]
        elif add.find("里") != -1:
            add5 = add[:add.find("里") + 1]
            add = add[add.find("里") + 1:]
        else:
            add5 = ""
        if add.find("号") != -1:
            if add.find("号楼") != -1:
                add6 = add[:add.find("楼") + 1]
                add = add[add.find("楼") + 1:]
            else:
                add6 = add[:add.find("号") + 1]
                add = add[add.find("号") + 1:]
        elif add.find("#") != -1:
            add6 = add[:add.find("#") + 1]
            add = add[add.find("#") + 1:]
        elif add.find("栋") != -1:
            add6 = add[:add.find("栋") + 1]
            add = add[add.find("栋") + 1:]
        # 仅数字无后缀
        elif re.findall(r'\d+', add):
            num = add.find("".join(re.findall(r'\d+', add)))
            add6 = add[:num + 1]
            add = add[num + 1:]
        else:
            add6 = ""
        add7 = add
        ans = [add1, add2, add3, add4, add5, add6, add7]
    return ans


# 处理数据
def data_processing(msg: str):
    # 判断级别类型并处理
    op_type = msg[0]
    msg = msg[2:]
    name, phone, address_ = div_name_phone(msg)
    address = div_address(address_, op_type)
    return json.dumps({"姓名": name, "手机": phone, "地址": address})
